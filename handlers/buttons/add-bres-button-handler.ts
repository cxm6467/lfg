...
export const addBresButtonHandler = async (client: Client, groupId: string, user: User) => {
	try {
		...
		if (!member) {
			logger(LogLevel.WARN, `User ${user.id} is not in group ${groupId}`, client.guilds.cache.map((guild) => guild.name).join(', '));
		...
		if (group?.members?.some(m => m.hasBres)) {
			logger(LogLevel.WARN, `Group with ID ${groupId} already has Bres`, client.guilds.cache.map((guild) => guild.name).join(', '));
		...
		if (embedMessage) {
			...
			logger(LogLevel.WARN, `Message for group ${groupId} not found`, client.guilds.cache.map((guild) => guild.name).join(', '));
			await user.send('Failed to update the group message embed.');
		}

	}
	catch (error) {
		logger(LogLevel.ERROR, `Error handling addBresButton: ${(error as Error).message}`, client.guilds.cache.map((guild) => guild.name).join(', '));
